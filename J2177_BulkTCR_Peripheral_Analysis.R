...
BulkTCR <- lapply(BulkTCR, function(df) {
  pick <- function(opts) intersect(opts, names(df))[1]
  
  st <- pick(c("sequenceStatus", "frame_type"))
  aa <- pick(c("aminoAcid", "amino_acid"))
  ct <- pick(c("count..templates.reads.", "templates"))
  vv <- pick(c("vMaxResolved", "v_resolved"))
  jj <- pick(c("jMaxResolved", "j_resolved"))
  
  df %>%
    # accept common “in-frame” spellings
    filter(grepl("^in", .data[[st]], ignore.case = TRUE),
           grepl("^C.*[FW]$", .data[[aa]])) %>%
    transmute(
      CDR3aa    = .data[[aa]],
      Counts    = .data[[ct]],
...
# For patient 01, different analysis since there were no baseline expansions
if(Patient_ID == "01"){
...
# Remove baseline for patient 01
timepoints <- meta %>%
  filter(Patient == Patient_ID) %>%
  filter(!(Patient_ID == "01" & Timepoint == "Baseline")) %>%
  pull(Timepoint) %>%
  unique()
...
# Identify the clonotypes that are pre-existing
baseline_unique <- unique(unlist(
  lapply(
    BulkTCR_unique[grepl("^Baseline", names(BulkTCR_unique))],
    function(df) df$Unique
  )
))

for (i in seq_along(BulkTCR_merged_sig_final)) {
  BulkTCR_merged_sig_final[[i]]$Pre_existing <-
    BulkTCR_merged_sig_final[[i]]$Unique %in% baseline_unique
}
...
for (sample_name in names(BulkTCR_merged)) {
  ...
  accent_col <- if (grepl("^Baseline", sample_name) & grepl("G12V", sample_name)) {"#643602"}
  else if (grepl("^Baseline", sample_name) & grepl("G12D", sample_name)) {"#00106b"}
  else if (grepl("G12V", sample_name)) {"#FB8808"}
...
#### E.3 Number of Pre-existing Clonotypes ####
Frequency <- cbind(Frequency, Pre_existing = NA)

for (df_name in names(BulkTCR_merged_sig_final)) {
  Frequency[df_name, "Pre_existing"] <- sum(BulkTCR_merged_sig_final[[df_name]]$Pre_existing, na.rm = TRUE)
}

#### E.4 Merge into meta_lim ####
...
clean_tcr_list <- function(tcr_list) {
  lapply(names(tcr_list), function(df_name) {
    df <- tcr_list[[df_name]]
    df <- df[, c("Unique", "CDR3aa", "vResolved", "jResolved", "Frequency.x", "Counts.x", "Counts.y", "Pre_existing")]
...
merge_tcr_list <- function(tcr_list) {
  ...
  merge_cols <- c("Unique", "CDR3aa", "vResolved", "jResolved", "Pre_existing")
...
if(Patient_ID == "01"){
  ...
  meta_lim <- meta_lim %>%
    mutate(Timepoint = factor(Timepoint, levels = c(timepoints))) %>%
    arrange(Timepoint)
}
